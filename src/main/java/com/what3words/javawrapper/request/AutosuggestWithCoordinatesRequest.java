...
public class AutosuggestWithCoordinatesRequest extends Request<AutosuggestWithCoordinates> {
    ...
    private AutosuggestWithCoordinates execute() {
        return super.execute(api.what3words().autosuggestWithCoordinates(input, nResults, focus, nFocusResults, clipToCountry, clipToBoundingBox, clipToCircle, clipToPolygon, inputType, language, locale, preferLand), AutosuggestWithCoordinates.class);
    }
    ...
    public static class Builder extends AbstractBuilder<AutosuggestWithCoordinates> {
        ...
        public Builder options(AutosuggestOptions options) {
            ...
            if (options.getLocale() != null) language(options.getLocale());
